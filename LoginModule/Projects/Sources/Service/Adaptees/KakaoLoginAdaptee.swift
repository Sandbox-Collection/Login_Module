...
public protocol KakaoLoginServiceInterface {
    func requestLogin() async throws -> LoginDTO
}


final class KakaoLoginService: NSObject, KakaoLoginServiceInterface {
            ...
            return try await withCheckedThrowingContinuation { continuation in
                UserApi.shared.loginWithKakaoTalk(
                    ...
                    guard let oauthToken
                        ...
                        continuation.resume(throwing: LoginError.invalidResponse)
                        return
                    }
                    print("oauthToken: \(oauthToken)")
                    
                    self.decodeIDToken(oauthToken.idToken!)
                    
                    continuation.resume(
                        returning: .init(
                            nonce: nonce,
                            response: KakaoLoginResponseDTO(oauthToken: oauthToken)
                        )
                    )
                }
            }
